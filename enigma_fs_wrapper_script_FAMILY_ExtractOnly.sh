...
# 10 October 2024
# Adapted by R Muetzel for FAMILY Consortium to only run step 1 and step 2 (exclude Matlab dependency)
...
`basename $0` --subjects <subject.txt> --fsdir </freesufer/path> --outdir <output/directory> --script <ENIGMA/scripts/path> --step_N 1 <other options>
...
if [[ -z ${subjectIDs} || -z ${fs_dir} ||  -z ${scripts_dir} || -z ${out_dir} || -z ${fs_version} ]]; then
	echo "ERROR: --subjects --fsdir, --script, --outdir are mandatory arguments. Please see usage: \n"
    Usage >&2
fi
